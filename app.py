...
def home():
    ...
    html += '</div></div><script>document.getElementById("uploadForm").addEventListener("submit",async function(e){e.preventDefault();const form=new FormData(e.target),btn=document.querySelector(".upload-btn");btn.disabled=true;btn.textContent="Uploading...";try{const response=await fetch("/upload",{method:"POST",body:form});if(!response.ok){throw new Error("Upload failed with status: "+response.status)}const contentType=response.headers.get("content-type");if(!contentType||!contentType.includes("application/json")){const text=await response.text();console.error("Non-JSON response:",text);throw new Error("Server error")}const result=await response.json();if(result.success){alert("Upload successful!");window.location.reload()}else{alert("Failed: "+(result.error||"Unknown error"))}}catch(error){console.error("Upload error:",error);alert("Error: "+error.message)}finally{btn.disabled=false;btn.textContent="Upload"}});</script></body></html>'
