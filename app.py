...
def listen(secret_id):
    ...
    html = '<!DOCTYPE html><html><head><title>' + track['title'] + '</title><meta name="viewport" content="width=device-width,initial-scale=1"><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700;800;900&display=swap" rel="stylesheet"><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Inter,sans-serif;background:#000;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px;color:#fff}.player{background:#0a0a0a;border-radius:20px;padding:40px;max-width:700px;width:100%;text-align:center;border:1px solid #1a1a1a}.cover{width:300px;height:300px;background:linear-gradient(135deg,#fe2c55 0%,#ff6b6b 100%);border-radius:16px;margin:0 auto 30px;display:flex;align-items:center;justify-content:center;font-size:7rem;box-shadow:0 20px 60px rgba(254,44,85,.3)}h1{font-size:2.2rem;margin-bottom:10px;font-weight:900;letter-spacing:-1px}.artist{color:#666;font-size:1.2rem;margin-bottom:30px;font-weight:700}.waveform-container{margin:30px 0;padding:25px;background:#0f0f0f;border-radius:12px;border:1px solid #1a1a1a}.waveform{display:flex;align-items:center;justify-content:center;gap:3px;height:100px}.bar{width:4px;background:linear-gradient(180deg,#fe2c55 0%,#ff6b6b 100%);border-radius:2px;animation:wave 1s ease-in-out infinite}.bar:nth-child(1){animation-delay:0s}.bar:nth-child(2){animation-delay:.05s}.bar:nth-child(3){animation-delay:.1s}.bar:nth-child(4){animation-delay:.15s}.bar:nth-child(5){animation-delay:.2s}.bar:nth-child(6){animation-delay:.25s}.bar:nth-child(7){animation-delay:.3s}.bar:nth-child(8){animation-delay:.35s}.bar:nth-child(9){animation-delay:.4s}.bar:nth-child(10){animation-delay:.45s}.bar:nth-child(11){animation-delay:.5s}.bar:nth-child(12){animation-delay:.55s}.bar:nth-child(13){animation-delay:.6s}.bar:nth-child(14){animation-delay:.65s}.bar:nth-child(15){animation-delay:.7s}.bar:nth-child(16){animation-delay:.75s}.bar:nth-child(17){animation-delay:.8s}.bar:nth-child(18){animation-delay:.75s}.bar:nth-child(19){animation-delay:.7s}.bar:nth-child(20){animation-delay:.65s}@keyframes wave{0%,100%{height:15px;opacity:.3}50%{height:100px;opacity:1}}.waveform.paused .bar{animation-play-state:paused;height:15px;opacity:.3}audio{width:100%;margin:20px 0;border-radius:8px}.download-btn{background:linear-gradient(135deg,#fe2c55 0%,#ff6b6b 100%);color:#fff;padding:16px 45px;border:none;border-radius:50px;font-size:1rem;font-weight:800;cursor:pointer;text-decoration:none;display:inline-block;transition:transform .2s;box-shadow:0 8px 25px rgba(254,44,85,.4);text-transform:uppercase;letter-spacing:1px}.download-btn:hover{transform:translateY(-3px);box-shadow:0 12px 35px rgba(254,44,85,.6)}.stats{margin-top:35px;display:flex;justify-content:center;gap:30px;color:#666;font-weight:700}.stat-item{padding:12px 24px;background:#0f0f0f;border-radius:10px;border:1px solid #1a1a1a;font-size:.9rem}</style></head><body><div class="player"><div class="cover">🎵</div><h1>' + track['title'] + '</h1><p class="artist">' + (track.get('artist') or 'Unknown Artist') + '</p><div class="waveform-container"><div class="waveform" id="waveform"><div class="bar"></div><div class="bar"></div><div class="bar"></div><div class="bar"></div><div class="bar"></div><div class="bar"></div><div class="bar"></div><div class="bar"></div><div class="bar"></div><div class="bar"></div><div class="bar"></div><div class="bar"></div><div class="bar"></div><div class="bar"></div><div class="bar"></div><div class="bar"></div><div class="bar"></div><div class="bar"></div><div class="bar"></div><div class="bar"></div></div></div><audio controls autoplay id="player"><source src="' + track['audio_url'] + '" type="audio/mpeg"></audio><button class="download-btn" onclick="downloadTrack()">Download Track</button><div class="stats"><div class="stat-item">' + str(track.get('plays', 0)) + ' plays</div><div class="stat-item">' + str(track.get('downloads', 0)) + ' downloads</div></div></div><script>const player=document.getElementById("player"),waveform=document.getElementById("waveform");player.addEventListener("play",function(){waveform.classList.remove("paused"),fetch("/api/increment/' + secret_id + '/plays",{method:"POST"})},{once:!0}),player.addEventListener("pause",function(){waveform.classList.add("paused")}),player.addEventListener("ended",function(){waveform.classList.add("paused")});function downloadTrack(){fetch("/api/increment/' + secret_id + '/downloads",{method:"POST"}),fetch("' + track['audio_url'] + '").then(e=>e.blob()).then(e=>{const a=document.createElement("a"),t=URL.createObjectURL(e);a.href=t,a.download="' + track['title'].replace("'", "") + '.mp3",document.body.appendChild(a),a.click(),document.body.removeChild(a),URL.revokeObjectURL(t)}).catch(()=>{window.open("' + track['audio_url'] + '","_blank")})}</script></body></html>'
